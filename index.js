...
async function extractImagesOnly(figFile, destDir) {
	...
	try {
		...
		debugLog('   Falling back to unzipper method...');
	}

	// Method 2: Fallback to unzipper (if ADM-ZIP fails)
	try {
		debugLog('🔧 Trying unzipper extraction method...');
		await extractWithUnzipper(figFile, destDir);
	} catch (e) {
		debugLog(`❌ Unzipper extraction also failed: ${e.message}`);
		log('❌ All extraction methods failed. The .fig file may be corrupted or use an unsupported format.');
	}
}

async function extractWithUnzipper(figFile, destDir) {
	const stream = fs.createReadStream(figFile);
	...
	unzipStream.on('entry', (entry) => {
		...
		allEntries.push(p);
		debugLog(`  Entry ${entryCount}: ${p} (${entry.type})`);

		if (entry.type === 'File' && p.startsWith('images/')) {
			imageCount++;
			const filename = p.substring('images/'.length);
			const outPath = path.join(destDir, filename);
			fs.mkdirSync(path.dirname(outPath), { recursive: true });
	...
	unzipStream.on('error', (error) => {
		throw new Error(`Unzip stream error: ${error.message}`);
	});

	// Wait for stream to finish with shorter timeout
	await new Promise((resolve, reject) => {
		...
		unzipStream.on('close', finish);
		unzipStream.on('end', finish);
		...
		setTimeout(() => {
			if (!resolved) {
				resolved = true;
				reject(new Error('Unzipper method timed out after 10 seconds'));
			}
		}, 10000);
	});
	...
	if (imageCount > 0) {
		log(`✅ Successfully extracted ${imageCount} images using unzipper`);
	}
}
